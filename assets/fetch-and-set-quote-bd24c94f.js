import{i as E,a as x}from"./vendor-ae6d56ab.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const s of n)if(s.type==="childList")for(const r of s.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&o(r)}).observe(document,{childList:!0,subtree:!0});function a(n){const s={};return n.integrity&&(s.integrity=n.integrity),n.referrerPolicy&&(s.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?s.credentials="include":n.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(n){if(n.ep)return;n.ep=!0;const s=a(n);fetch(n.href,s)}})();(()=>{const e=document.querySelector(".js-menu-container"),t=document.querySelector(".js-mobile-overlay"),a=document.querySelector(".js-open-menu"),o=document.querySelector(".js-close-menu"),n=()=>{e.classList.add("is-open"),t.classList.remove("hidden"),setTimeout(()=>{e.classList.remove("is-closing")},250)},s=()=>{e.classList.add("is-closing"),setTimeout(()=>{e.classList.remove("is-open","is-closing"),t.classList.add("hidden")},250)};a.addEventListener("click",n),o.addEventListener("click",s),t.addEventListener("click",r=>{r.target===t&&s()})})();const f={DOMEN:"https://your-energy.b.goit.study/api",FAV_KEY:"favorites"},R=(e,t=!1)=>e.length===0?'<p class="not-found-message">No results found.</p>':e.map(({_id:a,name:o,burnedCalories:n,rating:s,target:r,time:i,bodyPart:S})=>`<div class="exercises-container">
                ...
                <div class="exercises-info">
                    ...
                    <div class="exercises-info-item">
                        ...
                        <p class="exercises-info-value body-part">${S}</p>
                    </div>
                    <div class="exercises-info-item">
    ...
    </button>`}function j(e){return e%1?`${Math.round(e*10)/10}`:`${e}.0`}function N({params:e,pagination:t,method:a}){const o=Array.from(t.querySelectorAll(".button"));let n=1;o.forEach(s=>{s.addEventListener("click",r=>{s.classList.contains("active")||(n=s.dataset.page,a({...e,page:n}),o.forEach(i=>i.classList.remove("active")),s.classList.add("active")),window.innerWidth<768&&document.querySelector(".filter-title").scrollIntoView({behavior:"smooth"})})})}function k({params:e,totalPages:t,method:a}){const o=document.querySelector(".pagination");let n="";const s=Number(e.page);if(t<=1){o.innerHTML="";return}s>3&&(n+=`
  ...
  `,s+1<=t&&(n+=`
    ...
    `),o.innerHTML=n,N({params:e,pagination:o,method:a})}function l({type:e,title:t,message:a,position:o="topRight"}){switch(e){case"error":E.error({title:t,message:a,position:o});break;case"success":E.success({title:t,message:a,position:o});break;case"info":E.warning({title:t,message:a,position:o});break}}async function C(e){const t=document.querySelector(".loader-text");t.style.display="block";try{const{data:a}=await x({method:"get",url:`${f.DOMEN}/exercises/${e}`,responseType:"json"});return a}catch{l({type:"error",title:"Server error",message:"Sorry, the exercise information was not retrieved from the server. Please refresh the page"})}finally{t.style.display="none"}}let g,u;function H(e){g=document.getElementById("addFavoritesButton"),u=document.getElementById("removeFavoritesButton"),g.addEventListener("click",_),u.addEventListener("click",V),M(e)?w():F()}function J(){const e=document.getElementById("exerciseModal"),t=e.querySelector(".add-favorites-btn").getAttribute("data-id"),a=e.querySelector(".exercise-title-js").getAttribute("data-value"),o=Number(e.querySelector(".rating-value-js").getAttribute("data-value")),n=e.querySelector(".details-target-js").getAttribute("data-value"),s=e.querySelector(".details-body-part-js").getAttribute("data-value"),r=e.querySelector(".details-burned-calories-js").getAttribute("data-value"),i=Number(r.split("/")[0]),S=r.split("/")[1],A=Number(S.split(" ")[0]);return{_id:t,name:a,burnedCalories:i,rating:o,target:n,time:A,bodyPart:s}}function P(e){let t=b();t[e._id]=e,localStorage.setItem("favorites",JSON.stringify(t))}function M(e){return b()[e]||null}function _(){const e=J();M(e._id)||(P(e),w()),I()&&h({page:1})}function V(){let e=b();const t=u.dataset.id;e[t]&&(delete e[t],localStorage.setItem("favorites",JSON.stringify(e)),F()),I()&&h({page:1})}function Y(e){let t=b();const a=e.currentTarget.getAttribute("data-id");t[a]&&(delete t[a],localStorage.setItem("favorites",JSON.stringify(t)),h({page:1}))}function F(){g.classList.remove("hidden"),u.classList.add("hidden")}function w(){g.classList.add("hidden"),u.classList.remove("hidden")}function I(){return document.querySelector("body").classList.contains("js-favorites")}function b(){return JSON.parse(localStorage.getItem(f.FAV_KEY))||{}}async function K(e,t){const{data:a}=await x({method:"patch",url:`${f.DOMEN}/exercises/${e}/rating`,responseType:"json",data:t});return a}function W(){const e=document.getElementById("modalOverlay"),t=`
      ...
      <div class="exercise-details">
        ...
        <ul class="exercise-info">
          ...
          <li class="details-column">
            ...
            <div class="details-value">${p(e.equipment)}</div>
          </li>
          <li class="details-column">
